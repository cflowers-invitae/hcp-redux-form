...
describe('updateField', () => {
  ...
  it('should set pristine and dirty', () => {
    const result1 = updateField({}, {initial: 'foo', value: 'foo'}, false, undefined)
    expect(result1.dirty).toBe(false)
    expect(result1.pristine).toBe(true)
    const result2 = updateField({}, {initial: 'bar', value: 'foo'}, false, undefined)
    expect(result2.dirty).toBe(true)
    expect(result2.pristine).toBe(false)

    // test that it overwrites existing flags
    const result3 = updateField({dirty: true, pristine: false}, {initial: 'foo', value: 'foo'}, false, undefined)
    expect(result3.dirty).toBe(false)
    expect(result3.pristine).toBe(true)
    const result4 = updateField({dirty: false, pristine: true}, {initial: 'bar', value: 'foo'}, false, undefined)
  ...
  it('should prioritize submit error over async error', () => {
    expect(updateField({}, {asyncError: 'fooAsync', submitError: 'fooSubmit'}, false, undefined).error).toBe(
      'fooSubmit',
    )
    expect(updateField({}, {asyncError: 'barAsync', submitError: 'barSubmit'}, false, undefined).error).toBe(
      'barSubmit',
    )
  })
  ...
  it('should set valid/invalid', () => {
    ...
    // test that it overwrites existing flags
    const result3 = updateField({invalid: true, valid: false}, {}, false, undefined)
    expect(result3.valid).toBe(true)
    expect(result3.invalid).toBe(false)
    const result4 = updateField({invalid: false, valid: true}, {}, false, 'sync error')
  ...
  it('should change initial and default values when initial changes', () => {
    expect(updateField({defaultValue: 1, initialValue: 1}, {initial: 2}, false, undefined).initialValue).toBe(2)
    expect(updateField({defaultValue: 1, initialValue: 1}, {initial: 2}, false, undefined).defaultValue).toBe(2)
    expect(updateField({defaultValue: 1, initialValue: 1}, {initial: undefined}, false, undefined).initialValue).toBe(
      undefined,
    )
    expect(updateField({defaultValue: 1, initialValue: 1}, {initial: undefined}, false, undefined).defaultValue).toBe(
      undefined,
    )
  })
})
